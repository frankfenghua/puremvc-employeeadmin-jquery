...
},getSelectedRole:function(){return this.selectedRole;},setEnabled:function(isEnabled){var disabled=isEnabled?"":"disabled";
this.addRoleButton.attr("disabled",disabled);this.removeRoleButton.attr("disabled",disabled);
this.userRoleList.attr("disabled",disabled);this.roleList.attr("disabled",disabled);
if(!isEnabled){this.roleList.selectedIndex=-1;}},setMode:function(mode){switch(mode){case RolePanel.ADD_MODE:this.addRoleButton.attr("disabled","");
this.removeRoleButton.attr("disabled","disabled");break;case RolePanel.REMOVE_MODE:this.addRoleButton.attr("disabled","disabled");
this.removeRoleButton.attr("disabled","");this.roleList.selectedIndex=0;break;default:this.addRoleButton.attr("disabled","disabled");
this.removeRoleButton.attr("disabled","disabled");}},clearForm:function(){this.user=null;
this.setUserRoles(null);this.roleList.selectedIndex=0;this.userRoleList.jqGrid("resetSelection");
...
this.selectedRole=this.userRoles[index-1];this.setMode(RolePanel.REMOVE_MODE);},roleList_changeHandler:function(){this.userRoleList.selectedIndex=-1;
var roleEnumList=RoleEnum.getComboList();this.selectedRole=roleEnumList[this.roleList.attr("selectedIndex")];
if(this.selectedRole==RoleEnum.NONE_SELECTED){this.setMode(null);}else{this.setMode(RolePanel.ADD_MODE);
}}});RolePanel.ADD="add";RolePanel.REMOVE="remove";RolePanel.ADD_MODE="addMode";RolePanel.REMOVE_MODE="removeMode";
var UserForm=Objs("org.puremvc.js.demos.objs.employeeadmin.view.components.UserForm",UiComponent,{userFormPanel:null,uname:null,fname:null,lname:null,email:null,password:null,confirm:null,department:null,submitButton:null,user:null,userRoles:null,mode:null,roleListComparer:null,initialize:function(){UserForm.$super.initialize.call(this);
...
this.setFieldError("confirm",false);this.setFieldError("department",false);},setEnabled:function(isEnabled){var disabled=isEnabled?"":"disabled";
this.uname.attr("disabled",disabled);this.fname.attr("disabled",disabled);this.lname.attr("disabled",disabled);
this.email.attr("disabled",disabled);this.password.attr("disabled",disabled);this.confirm.attr("disabled",disabled);
this.department.attr("disabled",disabled);this.submitButton.attr("disabled",disabled);
this.cancelButton.attr("disabled",disabled);if(isEnabled&&this.mode==UserForm.MODE_EDIT){this.uname.attr("disabled","disabled");
...
break;}}this.selectedUser=uname;this.dispatchEvent(UserList.SELECT);},newButton_clickHandler:function(){this.dispatchEvent(UserList.NEW);
...
this.selectedUser=null;this.deleteButton.disabled=true;}});UserList.NEW="new";UserList.DELETE="delete";
UserList.SELECT="select";var RolePanelMediator=Objs("org.puremvc.js.demos.objs.employeeadmin.view.components.RolePaneMediator",Mediator,{roleProxy:null,initialize:function(name,viewComponent){RolePanelMediator.$super.initialize.call(this,RolePanelMediator.NAME,viewComponent);
